...
- support multiple return types as per API, so that if an API returns a 201, 400 or 422 and they all have different return
objects the `AxiosPromise` should be `AxiosPromise<A|B|C>` style.
...
and the type of generator is currently  `typescript-axios-enhanced`.
